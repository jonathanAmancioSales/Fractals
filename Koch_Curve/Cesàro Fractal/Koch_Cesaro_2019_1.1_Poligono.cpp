//=================================================
// Name        : Koch_Cesaro_2019_1.1_Poligono.cpp
// Author      : Jonathan Amancio
// Description : Cesaro Fractal [11.09.2019]
...
vector< Tripleto > Poligono(int P, Par O, float L, float angle){
	vector< Tripleto > list;
	float x,y, d_teta=360.0/P;
	for(int i=0; i<=P; i++)
	{	x=O.x+L*cos(Rad(angle));
		y=O.y+L*sin(Rad(angle));
		angle+=d_teta;
		list.push_back( {x,y,float(0.5*(180-d_teta)+angle)} );
	}	return list;
}
void Vetores_gnuplot(vector< Tripleto > list,double l,int iter,double angle,int P) {
	ofstream r;
	char str[30];
	sprintf(str,"T_%02d_%03.0f_%02d.txt",P,angle,iter);
	cout<<str;
	r.open(str, ios::app);
	//r.open(str);
	//r<<"A=\"set arrow from\"\n";
...
int main(int argc, char* argv[]){
	...
	float Ox=0, Oy=0, L=2.5;
	float angle_0=0;

	//new_p.push_back( {Ox,Oy,angle_0} );
	//new_p.push_back( {Ox+L*cos(Rad(angle_0)),Oy+L*sin(Rad(angle_0)),angle_0} );
	int P=atoi(argv[3]); new_p=Poligono(P,{Ox,Oy},L,90); L=2*L*sin(Rad(0.5*360.0/P));
	if(save_all==1) {Vetores_gnuplot(new_p,r,0,d_angle,P);}
	...
	{	r=L/(2.0*(1+sin(Rad(d_angle/2.0))));
		...
		if(save_all==1) {Vetores_gnuplot(new_p,L,i,d_angle,P);}
	}	if(save_all==0) {Vetores_gnuplot(new_p,L,n_iter,d_angle,P);}
	return 0;
}
